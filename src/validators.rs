...
pub fn tag_is_reserved_local(a: String) -> Result<(), String> {
    let local_tag_re: Regex = Regex::new(r"^[XYZ|a-z][[:alpha:]]$").unwrap();
    match local_tag_re.is_match(&a) {
        true => Ok(()),
        false => Err(String::from(
            "Your tag must be a valid tag reserved for local use. See SAMTags spec sheet.",
        )),
    }
}
...
pub fn vcf_seems_ok(a: String) -> Result<(), String> {
    ...
    let idx_exists = { Path::new(&tbi).is_file() || Path::new(&csi).is_file() };

    match p.is_file() {
        true => match is_acceptable_ext {
            true => match idx_exists {
                true => Ok(()),
                false => Err(String::from("Please index your vcf/bcf.")),
            },
            false => Err(String::from(
                "Input vcf/bcf does not appear to be a vcf/bcf.",
            )),
        },
